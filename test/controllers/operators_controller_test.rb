...
class OperatorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create operator" do
    assert_difference('Operator.count') do
      post operators_url, params: { operator: { name: @operator.name, operator_number: @operator.operator_number } }
    end
  ...
  test "should update operator" do
    patch operator_url(@operator), params: { operator: { name: @operator.name, operator_number: @operator.operator_number } }
